...
class Wheel(qtw.QGraphicsItem):
    def __init__(self, CenterX, radius=10, roady=10, parent=None, penTire=None, penMass=None, brushWheel=None,
                 brushMass=None, name='Wheel', mass=10):
        super().__init__(parent)
        self.x = CenterX
        self.road_y = roady
        self.radius = radius
        ...
        self.massBlock = MassBlock(CenterX, self.y, width=self.massWidth, height=self.massHeight, pen=penMass,
                                   brush=brushMass, name="Wheel Mass", mass=mass)
...
class LinearSpring(qtw.QGraphicsItem):
    def __init__(self, ptSt, ptEn, coilsWidth=10, coilsLength=30, parent=None, pen=None, name='Spring', label=None,
                 k=10, nCoils=6):
        """
...
class CarController():
    ...
    def doCalc(self, doPlot=True, doAccel=True):
        ...
        self.model.roadPosData = [self.model.ymag if t > self.model.tramp else t * self.model.ymag / self.model.tramp
    ...
    def OptimizeSuspension(self):
        ...
        # Step 1:
        #
        # Calculate spring force for both springs
        Fs1 = self.model.k1 * (self.model.x1 - self.model.x2)  # Force in spring 1
        Fs2 = self.model.k2 * self.model.x2  # Force in spring 2

        # Calculate dashpot force
        Fd1 = self.model.c1 * (self.model.v1 - self.model.v2)  # Damping force in dashpot

        # Update model forces (This might need to adjust based on actual variable names and logic in your model)
        self.model.Fs1 = Fs1
        self.model.Fs2 = Fs2
        self.model.Fd1 = Fd1

        # Ensure these forces are used in your model's dynamics calculations or plotting
